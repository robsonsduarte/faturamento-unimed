 ...
 * The Playwright page stays open waiting for submitToken().
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cookies) {
      ...
      if (!loginResult.success) return NextResponse.json({ error: `Login SAW falhou: ${loginResult.error}` }, { status: 502 })
